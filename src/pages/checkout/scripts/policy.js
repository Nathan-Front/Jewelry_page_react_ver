export const returnPolicyData = [
  {
    title: "Return Eligibility",
    items: [
      "You may request a return within 14 days of delivery if:",
      "The item is unused and unworn",
      "It is in its original condition and packaging",
      "You have proof of purchase (order confirmation or receipt)",
    ],
  },

  {
    title: "Non-Returnable Items",
    items: [
      "For hygiene and customization reasons, the following items are not eligible for return:",
      "Earrings (for pierced ears)",
      "Customized or engraved jewelry",
      "Final sale or clearance items",
    ],
  },

  {
    title: "Exchanges",
    items: [
      "We currently offer exchanges for:",
      "Defective or damaged items",
      "Incorrect items received",
      "If you need a different size or style, we recommend placing a new order and returning the original item.",
    ],
  },
];